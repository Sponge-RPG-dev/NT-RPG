...
public class DamageService {
    ...
    public void recalculateCharacterWeaponDamage(IActiveCharacter character) {
        ...
        character.setCharacterProperty(DefaultProperties.current_weapon_damage, (float) damage);
    }

    public double getCharacterResistance(IActiveCharacter character, DamageSource source) {
        DamageModifierBuilder b;
        if (source == DamageSources.MAGIC)
            return character.getCharacterProperty(DefaultProperties.magic_damage_protection_mult);
 /*       if (source == DamageSources.LIGHTING)
            return character.getCharacterProperty(DefaultProperties.lightning_damage_protection_mult);
   */
        if (source == DamageSources.WITHER)
            return character.getCharacterProperty(DefaultProperties.wither_damage_protection_mult);
        if (source == DamageSources.FIRE_TICK || source == DamageSources.IN_FIRE)
