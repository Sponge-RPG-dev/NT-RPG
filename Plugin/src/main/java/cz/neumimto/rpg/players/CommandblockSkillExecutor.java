...
public class CommandblockSkillExecutor extends PreloadCharacter {
	...
	private CommandblockSkillExecutor(CommandBlockSource cb) {
		super(null);
		this.c = cb;
		stub = new DummyPlayer(cb.getLocation());
	}

	public static CommandblockSkillExecutor wrap(CommandBlockSource commandBlock) {
	...
	private static class DummyPlayer implements Player {
		...
		public DummyPlayer(Location<World> location) {
		...
		public void spawnParticles(ParticleEffect particleEffect, Vector3d position) {
			position = getLocation().getPosition().add(position);
			spawnParticles(particleEffect, position, 0);
		}

		@Override
		public void spawnParticles(ParticleEffect particleEffect, Vector3d position, int radius) {
			position = getLocation().getPosition().add(position);
			getLocation().getExtent().spawnParticles(particleEffect, position, radius);
		}

		@Override
		public void playSound(SoundType sound, SoundCategory category, Vector3d position, double volume) {
			position = getLocation().getPosition().add(position);
			playSound(sound, category, position, volume, 0);
		}

		@Override
		public void playSound(SoundType sound, SoundCategory category, Vector3d position, double volume, double pitch) {
			position = getLocation().getPosition().add(position);
			playSound(sound, category, position, volume, 0, 0);
		}
		...
		public void playSound(SoundType sound, SoundCategory category, Vector3d position, double volume, double pitch, double minVolume) {
			position = getLocation().getPosition().add(position);
			getLocation().getExtent().playSound(sound, category, position, volume, pitch, minVolume);
		}
